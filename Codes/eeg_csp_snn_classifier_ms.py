...
def main():
    ...
    def load_all_subjects(subject_ids):
        ...
        for sid in subject_ids:
            for session_type in ["T", "E"]:
                X, y = load_data(args.base_dir, sid, session_type)
    ...
    X_train, y_train = load_all_subjects(train_subs)
    X_test, y_test = load_all_subjects([args.test_sub])

    csp = PairwiseCSP(n_components=X_train.shape[1], selected_classes=[1, 2, 3, 4], reg_lambda=args.lambda_R)
    ...
    projected_train = csp.transform(X_train)
    projected_test = csp.transform(X_test)
