...
class MyCustomLIF(nn.Module):
    ...
    def forward(self, input, mem):
        mem = self.beta * mem + input
        spike = (mem >= self.threshold).float()
        mem = mem * (1 - spike)
        return spike, mem
    def init_mem(self, batch_shape):
...
class HodgkinHuxleyNeuron(nn.Module):
    ...
    def forward(self, I_inj, state):
        ...
        dV = (I_inj - INa - IK - IL) / self.Cm
        V = V + self.dt * dV
        spike = (V >= 0).float()
        V = V * (1 - spike) + (-65.0) * spike
        return spike, (V, m, h, n)
    def init_state(self, batch_shape=()):
        V = torch.full(batch_shape, -65.0, device=self.device)
        m = torch.full(batch_shape, 0.05, device=self.device)
        h = torch.full(batch_shape, 0.6, device=self.device)
        n = torch.full(batch_shape, 0.32, device=self.device)
        return (V, m, h, n)

# 3. SNN Classifier with Pluggable Neuron
class SNNClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, output_size,
                 population_per_class=5, dropout_prob=0.5,
                 neuron_type='leaky', neuron_kwargs=None, device='cpu'):
        super().__init__()
        ...
        # Neuron selection logic
        neuron_type = neuron_type.lower()
        neuron_kwargs = neuron_kwargs or {}
        if neuron_type == 'leaky':
            neuron_class = snn.Leaky
        elif neuron_type == 'lif':
            neuron_class = snn.LIF
        elif neuron_type == 'if':
            neuron_class = snn.IF
        elif neuron_type == 'plif':
            neuron_class = snn.PLIF
        elif neuron_type == 'alif':
            neuron_class = snn.ALIF
        elif neuron_type == 'izhikevich':
            neuron_class = snn.Izhikevich
        elif neuron_type == 'customlif':
            neuron_class = MyCustomLIF
        elif neuron_type == 'hh':
        ...
        if neuron_type == 'hh':
            self.neuron1 = neuron_class(device=device)
            self.neuron2 = neuron_class(device=device)
        elif neuron_type == 'customlif':
            self.neuron1 = neuron_class(**neuron_kwargs)
            self.neuron2 = neuron_class(**neuron_kwargs)
        else:
            self.neuron1 = neuron_class(**neuron_kwargs)
            self.neuron2 = neuron_class(**neuron_kwargs)
    def forward(self, x):
        # x shape: (time_steps, batch, input_size)
        time_steps, batch_size, _ = x.shape
        # For HH, need state tuples (V, m, h, n), otherwise use mem
        if isinstance(self.neuron1, HodgkinHuxleyNeuron):
            state1 = self.neuron1.init_state((batch_size, self.fc1.out_features))
            state2 = self.neuron2.init_state((batch_size, self.fc2.out_features))
        elif isinstance(self.neuron1, MyCustomLIF):
            state1 = self.neuron1.init_mem((batch_size, self.fc1.out_features))
            state2 = self.neuron2.init_mem((batch_size, self.fc2.out_features))
        else:
            state1 = self.neuron1.init_leaky()
            state2 = self.neuron2.init_leaky()
        rec_spk2 = []
        for t in range(time_steps):
            out1 = self.fc1(x[t])
            out1 = self.dropout1(out1)
            if isinstance(self.neuron1, HodgkinHuxleyNeuron):
                spk1, state1 = self.neuron1(out1, state1)
            else:
                spk1, state1 = self.neuron1(out1, state1)
            out2 = self.fc2(spk1)
            out2 = self.dropout2(out2)
            if isinstance(self.neuron2, HodgkinHuxleyNeuron):
                spk2, state2 = self.neuron2(out2, state2)
            else:
                spk2, state2 = self.neuron2(out2, state2)
            rec_spk2.append(spk2)
        return torch.stack(rec_spk2)

# --------- DEMO: Instantiate Model With Different Neuron Types ---------
if __name__ == "__main__":
    ...
    hidden_size = 16
    output_size = 4
    population_per_class = 3
    x = torch.randn(time_steps, batch_size, input_size)
    # Select neuron: 'leaky', 'lif', 'if', 'plif', 'alif', 'izhikevich', 'customlif', 'hh'
    neuron_type = 'hh'  # or 'customlif', etc.
    neuron_kwargs = {}  # e.g., {"beta": 0.9} for LIF/PLIF
    model = SNNClassifier(input_size, hidden_size, output_size,
                         ...
                         device='cpu')
    y = model(x)
    print(f"Output shape (time, batch, outputs): {y.shape}")
...
if __name__ == "__main__":
    ...
    neuron_type = 'leaky'        # Standard LIF neuron
    #neuron_type = 'lif'          # Continuous LIF
    #neuron_type = 'if'           # Integrate-and-fire (no leak)
    #neuron_type = 'plif'         # Parameterized LIF (learns beta)
    #neuron_type = 'alif'         # Adaptive LIF (SFA)
    #neuron_type = 'izhikevich'   # Izhikevich
    #neuron_type = 'hh'           # Hodgkin-Huxley
