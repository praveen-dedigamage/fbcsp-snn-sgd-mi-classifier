...
def train_with_ideal_spikes(model, X_train, y_train, X_val, y_val, LR=1e-3, epochs=10, target_spike_probability = 0.7):
    ...
    early_stopper = EarlyStopping(patience=100, min_delta=1e-4, mode='min')  # monitor val_acc
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if epoch > 1000:
                ...
                if early_stopper.early_stop:
                    print(f"Early stopping triggered at epoch {epoch+1} with val_acc={val_acc:.4f}")
                    break
        ...
        print("Epoch Time", time.time() - estart)
...
if __name__ == "__main__":
    ...
    train_acc, train_cm = evaluate(best_model, spike_train_train, torch.tensor(y_train - 1))
    test_acc, test_cm = evaluate(best_model, spike_train_val, torch.tensor(y_val - 1))

    results_dir = os.path.join(base_directory, f"results_{subjectID}")
    os.makedirs(results_dir, exist_ok=True)
    save_path = f'model_and_history_LR{lambda_R}_FB{freq_bands}_SP{spiking_prob}_BaseThres{base_thresh_val}_AdaptInc{adapt_inc_val}_Decay{decay_val}_HN{hidden_number_of_Neuron}_NPPC{neuro_population_per_class}_SubID{subjectID}.pth'
