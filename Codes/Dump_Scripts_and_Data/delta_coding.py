...
# -------------------------- Spike Encoding --------------------------
def encode_projected_signals_to_spikes(projected_data_dict, threshold=0.05, seed=None):
    if seed is not None:
        torch.manual_seed(seed)
    all_encoded = []
    for pair in sorted(projected_data_dict):
        data = projected_data_dict[pair]  # shape: (batch, channels, time)
        tensor_data = torch.tensor(data).float()  # Convert to torch.Tensor
        tensor_data = tensor_data.permute(2, 0, 1)  # shape: (time, batch, channels)
        spike_tensor = spikegen.delta(tensor_data, threshold=threshold)
