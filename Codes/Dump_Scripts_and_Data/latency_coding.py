...
# -------------------------- Spike Encoding --------------------------
def encode_projected_signals_to_spikes(projected_data_dict, time_steps=100, seed=None):
    if seed is not None:
        torch.manual_seed(seed)
    all_encoded = []
    for pair in sorted(projected_data_dict):
        ...
        norm_tensor = torch.tensor(norm_variance).float()
        spike_tensor = spikegen.latency(norm_tensor, num_steps=time_steps)
        all_encoded.append(spike_tensor)
...
if __name__ == "__main__":
    ...
    # Spike encoding
    time_steps = 100
    spike_train_train = encode_projected_signals_to_spikes(projected_train, time_steps=time_steps)
    spike_train_val = encode_projected_signals_to_spikes(projected_val, time_steps=time_steps)
